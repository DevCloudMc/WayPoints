...
class PermissionLimitTest {
    ...
    fun `returns highest matching permission`() {
        val p = mockk<Permissible>()
        every { p.effectivePermissions } returns
            mutableSetOf(
    ...
    fun `ignores negated permissions`() {
        val p = mockk<Permissible>()
        every { p.effectivePermissions } returns
            mutableSetOf(permInfo("waypoints.limit.10", false), permInfo("waypoints.limit.5", true))
    ...
    fun `ignores non-numeric suffixes`() {
        val p = mockk<Permissible>()
        every { p.effectivePermissions } returns
            mutableSetOf(permInfo("waypoints.limit.foo", true), permInfo("waypoints.limit.4", true))
