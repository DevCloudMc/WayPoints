...
private class StorageTypeValidator : InputValidator<StorageType> {
    ...
    override fun getTabCompletes(sender: CommandSender, args: Array<out String>): Collection<String> =
        StorageType.values().map { it.name.lowercase() }
}
...
class MigrateCommand(private val ctx: WayPointsBootstrap) {
    ...
    private fun execute(sender: CommandSender, args: Array<out String>, p: CommandParameters) {
        ...
        val source: StorageBackend =
            when (from) {
                StorageType.SQLITE -> SqliteStorageBackend(dataFolder.resolve(ctx.config.sqliteFile))
        ...
        source.init().thenCompose { source.exportAll() }
            .thenCompose { snapshot -> target.init().thenCompose { target.importAll(snapshot) }.thenApply { snapshot } }
            .whenComplete { snapshot, err ->
                ctx.async.runOnMain {
                    ...
                    if (err != null) {
                        ctx.messenger.send(
                            sender,
                            ctx.lang.message("admin-migrate-failed", "reason" to (err.message ?: err.toString())),
                        )
                    } else {
                        ctx.messenger.send(
                            sender,
